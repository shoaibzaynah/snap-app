        ...
        <div className="absolute bottom-2 left-2 z-10 w-[calc(100%-54px)] sm:w-auto sm:max-w-xs bg-[#0B0B0E]/95 backdrop-blur-xl border border-white/10 rounded-xl p-2 shadow-2xl space-y-1.5">
          <div className="flex items-center justify-between gap-1.5">
            <div className="flex items-center gap-1 min-w-0">
              <span className="text-[11px] font-mono font-bold text-[#FFFC00] truncate">{latest.latitude.toFixed(4)}, {latest.longitude.toFixed(4)}</span>
              <span className="text-[9px] text-white/50 font-mono shrink-0">{latest.accuracy ? `±${Math.round(latest.accuracy)}m` : "GPS"}</span>
            </div>
            <a href={`https://www.google.com/maps?q=${latest.latitude},${latest.longitude}`} target="_blank" rel="noopener noreferrer" className="h-6 px-2 rounded-md bg-[#FFFC00] hover:bg-[#FFFC00]/90 text-black font-black text-[10px] shrink-0 transition-all active:scale-95 shadow-sm flex items-center gap-1" title="Open Google Maps">
