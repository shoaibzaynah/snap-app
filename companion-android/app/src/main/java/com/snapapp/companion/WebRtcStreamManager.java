...
public class WebRtcStreamManager {
    ...
    private String activeServerUrl, activeDeviceId; private PowerManager.WakeLock wakeLock; private WifiManager.WifiLock wifiLock;
    ...
    public synchronized void setAudioOutput(String mode) {
        ...
        WebRtcIceHelper.setAudioOutput(appContext, useSpeaker);
    }
    ...
    public synchronized void startLiveStream(final Context ctx, final String serverUrl, final String deviceId,
        ...
        this.activeServerUrl = serverUrl; this.activeDeviceId = deviceId; this.isFrontCamera = front; this.useSpeaker = speakerOn;
        acquireLocks(ctx);
        try {
            WebRtcIceHelper.setAudioOutput(ctx, useSpeaker);
            new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
                if (appContext != null) WebRtcIceHelper.setAudioOutput(appContext, useSpeaker);
            }, 600);
    ...
    private void acquireLocks(Context ctx) {
        try {
            ...
            if (pm != null && (wakeLock == null || !wakeLock.isHeld())) (wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "snap:live_stream")).acquire(15 * 60 * 1000L);
            WifiManager wm = (WifiManager) ctx.getApplicationContext().getSystemService(Context.WIFI_SERVICE);
            if (wm != null && (wifiLock == null || !wifiLock.isHeld())) (wifiLock = wm.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "snap:wifi_stream")).acquire();
                ...
                peerConnection.createAnswer(new SimpleSdpObserver() {
                    @Override public void onCreateSuccess(SessionDescription answer) {
                        ...
                        peerConnection.setLocalDescription(new SimpleSdpObserver() {
                            @Override public void onSetSuccess() {
                                // Wait for ICE gathering complete before sending answer
    ...
    public synchronized void handleRemoteCandidate(String sdp, int sdpMLineIndex, String sdpMid) {
        ...
        if (hasRemoteDesc && peerConnection.getRemoteDescription() != null) { try { peerConnection.addIceCandidate(c); } catch (Exception ignored) {} }
